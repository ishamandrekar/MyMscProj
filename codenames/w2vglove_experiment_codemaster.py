...
NUM_EXP = 30
CODEMASTER = ["players.codemaster_w2vglove_07 --w2v vectors/GoogleNews-vectors-negative300.bin --glove vectors/glove.6B.300d.txt"]
...
OUTPUT_FILE = "w2vglovecmnewgs_results.csv"
...
for i in range(len(CODEMASTER)):
	for j in range(len(GUESSER)):
		...
		for t in range(NUM_EXP):
			...
			cmd = 'python game.py ' + CODEMASTER[i] + ' ' + GUESSER[j] + ' --board boards/board_' + str(t+1) + ".txt"
