...
class Game:
	...
	def run(self):
		...
		while game_condition != "Lose" or game_condition != "Win":
			...
			clue, num = self.codemaster.give_clue(game_condition) #Parameter Added for optimization
			game_counter += 1
			keep_guessing = True
			guess_num = 0
			num = int(num)
			...
			game_condition = "Hit_Red"
			while (guess_num <= num) and (keep_guessing == True) and (game_condition == "Hit_Red"):
				self.guesser.get_board(words_in_play)
				...
				if game_condition == "Hit_Red":
					...
					words_left_for_clue = self.guesser.keep_guessing(clue, words_in_play)
					keep_guessing = words_left_for_clue > 0
					print("Keep Guessing? "+ str(keep_guessing))
					if keep_guessing:
