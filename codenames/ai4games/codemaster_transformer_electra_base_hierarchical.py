...
class ai_codemaster(codemaster):
    ...
    def __init__(self, brown_ic=None, glove_vecs=None, word_vectors=None):
        ...
        with torch.no_grad():
            self.tokenizer = ElectraTokenizer.from_pretrained('google/electra-base-generator')
            self.model = ElectraModel.from_pretrained('google/electra-base-generator', output_hidden_states = False,)
            self.model.eval()
        ...
        self.nnfit = 0
        self.pca = PCA(n_components=256, svd_solver='auto')
    ...
    def give_clue(self, game_condition):
        ...
        if len(self.all_guesses)==0:
            ...
            self.vectors = self.reduce_dimentions_pca(emb_matrix)
        t2 = time.perf_counter()
        print("embedding creation "+ str(t2-t1))        
        ...
        t2 = time.perf_counter()    
        print("vacablary embeddings extraction "+ str(t2-t1)) 
        ...
        if 1==1: 
            ...
            if len(self.best_combs)==0:
                if len(current_red) != 1:
                    label .repeat_with_more_neighbours
                    ...
                    mergings = linkage(torch.flatten(all_current_board_word_embeddings, start_dim=1).cpu().detach().numpy(), method = "single", metric='euclidean')
                    list_cluster_groups = []
                    for num_clusters in range(2,total_words+1):
                        list_cluster_groups.append(list(cut_tree(mergings, n_clusters = num_clusters).reshape(-1,)))
                      
                    #logical steps for choosing the best groups
                    #1. Iterate through all dendrogram cuttings and explore the groups formed
                    #2. Condition1: choose red groups which do not include any bad words(civil+blue+assasin). Iterate all dendrogram cuttings to find groups which satisfy Condition1, and choose them as the best groups.
                    #3. If condition1 satisfied for only when each red word is part of seperate group, i.e. no of groups = len(current_red), relax Condition1 a bit(Condition2)
                    #4  Condition2: choose red groups such that civil words allowed as part of thegroups but not blue and assasin. Iterate all dendrogram cuttings to find groups which satisfy Condition2, and choose them as the best groups.
                    #5  If condition2 satisfied for only when each red word is part of seperate group, i.e. no of groups = len(current_red), relax Condition2 a bit(Condition3)
                    #6 Condition3: choose red groups such that civil and blue words allowed as part of the groups but not assasin word. Iterate all dendrogram cuttings to find groups which satisfy Condition3. If still Condition3 satisfied when each red word part of seperate group, i.e  no of groups = len(current_red), choose them as last resort as the best groups.
                    
                    for labels in list_cluster_groups:
                        ...
                        if len(curr_best_red_combs) < len(current_red) and (len(curr_best_red_combs)>0):
                            break
                        elif (len(curr_best_red_combs) == len(current_red)) and (len(current_red) <= 3):
                            break
                    
                    if (len(curr_best_red_combs) == len(current_red)) and (len(current_red) > 3):
                        for labels in list_cluster_groups:
                            current_label = labels
                            curr_best_red_combs = []
                            for element in labels[:len(current_red)]:
                    ...
                    for unq_item in list(np.unique([elem[0] for elem in curr_best_red_combs])):#list(np.unique(labels)):
                             red_combs_curr.append(list(locate(current_label[:len(current_red)], lambda x: x == unq_item)))
                    
                    for red_comb in red_combs_curr:
                        self.red_combs.append([current_red[element] for element in red_comb if current_red[element] not in self.red_combs])                 
                ...
                if 1==1:
                    ...
                    self.best_combs = []
                    distance_order = []
                    #self.red_combs = list(set(self.red_combs))
                    unique = []
                    for item in self.red_combs:
                        if item not in unique:
                            unique.append(item)
                    self.red_combs = unique
                    ...
                    for comb_indx, comb in enumerate(self.red_combs):
                        found_old = 0
                        if (comb in old_red_combs) and (self.repeat==0):
        ...
        if found == 0:
            for element in self.best_combs[::-1]:
                if (element[3] not in self.all_guesses):
                    clue = element[3]
                    clue_num = element[1]
                    found = 1
                    break
        ...
        if found == 0:
            print("9")
            self.no_of_neighbours = self.no_of_neighbours + 500
            ...
            print(self.red_combs)
            goto .repeat_with_more_neighbours
    ...
    def word_embedding(self, red_words):
        with torch.no_grad():
            tokenized_texts = [self.tokenizer("[CLS] " + word + " [SEP]") for word in red_words]
            max_len = 0
            outputs_l = []
    ...
    def train_pca_model(self):
        emb_matrix = self.model.embeddings.word_embeddings.weight
        self.pca.fit_transform(emb_matrix.cpu().detach().numpy())
        
    def reduce_dimentions_pca(self, tensor_2d):
        output = torch.tensor(self.pca.transform(tensor_2d.cpu().detach().numpy()))
        return output
    ...
    def getBestCleanWord(self, center, board):
        tries = 1
        amt = self.no_of_neighbours #500
        maxTry = 1
        if (len(self.all_guesses)==0 and self.nnfit == 0) or (self.repeat>0):
