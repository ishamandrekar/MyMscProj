    ...
    def give_clue(self, game_condition):
        ...
        if len(self.all_guesses)==0:
            ...
            if len(self.blue_words) > 0:
                ...
                self.blue_emb_reduced = self.reduce_dimentions_pca(self.blue_emb)
            if len(self.assassin_word) > 0:
                self.assassin_emb = self.word_embedding(self.assassin_word)
                self.assassin_emb_reduced = self.reduce_dimentions_pca(self.assassin_emb)
            if len(self.civil_words) > 0:
                self.civil_emb = self.word_embedding(self.civil_words)
                self.civil_emb_reduced = self.reduce_dimentions_pca(self.civil_emb)
    ...
    def word_embedding(self, red_words):
        with torch.no_grad():
            ...
            for d1 in tokenized_texts:
                ...
                output = self.model.forward(tokens_tensor, segments_tensor)
                del tokens_tensor
                del segments_tensor
