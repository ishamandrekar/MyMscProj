...
class ai_codemaster(codemaster):
    ...
    def __init__(self, brown_ic=None, glove_vecs=None, word_vectors=None):
        ...
        with torch.no_grad():
            self.tokenizer = XLNetTokenizer.from_pretrained('xlnet-large-cased', from_tf=True)
            self.model = XLNetModel.from_pretrained('xlnet-large-cased', from_tf=True)
            self.model.eval()
...
###############
        ...
        if 1==1:#len(self.all_guesses)==0: 
            ...
            if len(self.best_combs)==0:
                #print("I'm Here5")
                if len(current_red) != 1:
                    clustering = AffinityPropagation(affinity='euclidean',random_state=5).fit(torch.flatten(self.red_emb[current_red], start_dim=1).cpu().detach().numpy())
                    ...
                    for unq_item in list(np.unique(labels)):
                        red_combs_curr.append(list(locate(labels, lambda x: x == unq_item)))
