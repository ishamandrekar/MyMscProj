...
###############
        ...
        if 1==1:#len(self.all_guesses)==0: 
            ...
            if len(self.best_combs)==0:
                #print("I'm Here5")
                if len(current_red) != 1:
                    #all_current_board_word_embeddings = torch.stack(self.red_emb[current_red],self.blue_emb[current_blue],self.civil_emb[current_civil],self.assassin_emb)
                    #print(self.red_emb[current_red].size())
                    #print(self.blue_emb[current_blue].size())
                    #print(self.civil_emb[current_civil].size())
                    #print(self.assassin_emb.size())
                    ...
                    all_current_board_word_embeddings =  torch.cat((red_emb_padded,blue_emb_padded,civil_emb_padded,assassin_emb_padded), 0)
                    #print(all_current_board_word_embeddings.size())
                    total_words = all_current_board_word_embeddings.size()[0]
                    map_pos_list = ['r']* len(current_red)  + ['b'] * len(current_blue) + ['b'] * len(current_civil) + ['b'] * 1
                    print(map_pos_list)
                    ...
                    for num_clusters in range(2,total_words+1):
                        ...
                        print(list(cut_tree(mergings, n_clusters = num_clusters).reshape(-1,)))
                        
                    all_best_red_combs = []
                    for clusters in list_cluster_groups:
                        print(clusters)
                        print(map_pos_list)
                        curr_best_red_combs = []
                        for element in clusters[:len(current_red)]:
                            if (element not in  clusters[len(current_red):]) and (element not in [elem[0] for elem in curr_best_red_combs]):
                                curr_best_red_combs.append((element,clusters.count(element)))
                        all_best_red_combs.append(curr_best_red_combs)        
                    
                  
                    max_group_len = 0
                    max_groups = 0
                    for comb in all_best_red_combs:
                        if len(comb) > 0:
                            if len(comb) > max_groups:
                                max_groups = len(comb)
                            if max_group_len > max([elem[1] for elem in curr_best_red_combs]):  
                                max_group_len = max([elem[1] for elem in curr_best_red_combs])
                    
                    
                    if max_group_len>1:
                    #if any comb having more than 1 elements choose that one
                       best_comb = [combs for comb in combs for combs_num, combs in enumerate(all_best_red_combs) if comb[1]==max_group_len][0]
                    else:
                    #else choose one which has formed maximum number of groups    
                       best_comb = [combs for combs_num, combs in enumerate(all_best_red_combs) if len(combs)==max_groups][0]
                    print("best_comb")
                    print(best_comb)
                    ...
                    for unq_item in list(np.unique([elem[0] for elem in best_comb])):
                        red_combs_curr.append(list(locate([elem[0] for elem in best_comb], lambda x: x == unq_item)))
