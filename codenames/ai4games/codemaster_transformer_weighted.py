...
###############
        ...
        spec_assassin_word = list(map(lambda w: "\u0120" + w, assassin_word))
        ...
        #print(("spec_red_words: "+ str(spec_red_words)).encode("utf-8"))
        if len(spec_red_words) > 0:
            self.red_emb = self.word_embedding(spec_red_words)  #retrieves embedding for red_words from gpt2 layer 0 (static embedding)
        #print(("spec_blue_words: "+ str(spec_blue_words)).encode("utf-8"))
        if len(spec_blue_words) > 0:
            self.blue_emb = self.word_embedding(spec_blue_words)
        #print(("spec_assassin_word: "+ str(spec_assassin_word)).encode("utf-8"))
        if len(spec_assassin_word) > 0:
            self.assassin_emb = self.word_embedding(spec_assassin_word)
        #print(("spec_civil_words: "+ str(spec_civil_words)).encode("utf-8"))
        if len(spec_civil_words) > 0:
        ...
        # create distance matrix for red words
        ...
        ## find the red word with more red word neighbors within threshold

        # count number of red neighbors below threshold for each red word
        #Here there is a mistake n code the sign should be <= as we are checking less than or equal to threshhold
        how_many = []
        for i in range(num_words):
            how_many.append((dist[i] >= self.dist_threshold).sum())

        #max number of red words clustered together
        clue_num = max(how_many)

        # find which is the red word with max number of red neighbors
    ...
    def getBestCleanWord(self, center, board):
        ...
        return self.weightWords(clean_words,low_board, center)
...
###### new code
    def weightWords(self, rec_words, low_board, center):
        ...
        for i in range(num_recomm):
            sim_center[i] = self.cos_sim(recomm6_vec[i],center)
